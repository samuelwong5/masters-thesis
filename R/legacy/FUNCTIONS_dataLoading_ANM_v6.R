...
# v6 = Added extraction of new brain volumes from Sarah for ANM
#
...
expandSubjects = function( exp_iSiG,
                           subjectID_iS ){
  #
  # Expands the number of rows in "exp_iSiG" for it to match all the subjects in "subjectID_iS". New rows are filled with NA
  #
  stopifnot( all( dimnames( exp_iSiG )[[1]] %in% subjectID_iS ) )
  exp_e_iSiG <- array( data = NA,
                       dim = c( length( subjectID_iS ),
                                dim( exp_iSiG )[2] ),
                       dimnames = list( subject = subjectID_iS,
                                        gen = dimnames( exp_iSiG )[[2]] ) );
  for( nS in dimnames( exp_iSiG )[[1]] ){
    exp_e_iSiG[nS, ] <- exp_iSiG[nS, ]
  }
  return( exp_e_iSiG )
}
...
loadANM_imag = function( subjectID_iS = NA,
                         correctNames = TRUE ){
  
  # Set default input if not IDs provided
  if( length( subjectID_iS ) == 1 
      && is.na( subjectID_iS ) ){
    subjectID_iS <- unique( imaging_iSiV[ ,"Individual"] )
  }
  
  # Load demographics for the required subjects
  print( "Loading brain imaging" );
  imaging_iSiV    <- read.csv( "G:\\Data\\ANM\\Imaging from Sarah\\sMRI_freesurfer_2017_hiveDataQuery.csv", 
                               header = TRUE, 
                               stringsAsFactors = FALSE );
  subjectID_iS <- fixSampleNames( nSs = subjectID_iS );
  subjectID_imag_iS <- fixSampleNames( nSs = imaging_iSiV[ ,"Individual"] );
  bGood_iV <- subjectID_imag_iS %in% subjectID_iS
  imaging_iSiV <- imaging_iSiV[ bGood_iV, ] 
  
  # Correct for ICV
  bCorrect_iV <- grepl( "_volume$",
                        dimnames( imaging_iSiV )[[2]] )
  imaging_k_iSiV <- imaging_iSiV
  meanICV <- mean( imaging_iSiV[ ,"EstimatedTotalIntraCranialVol"],
                   na.rm = TRUE )
  for( iV in which( bCorrect_iV ) ){
    # Apply regression method for correction
    print( paste( "Correcting", dimnames( imaging_iSiV )[[2]][iV] ) )
    r <- lm( formula = as.formula( paste( dimnames( imaging_iSiV )[[2]][iV],
                                          " ~ EstimatedTotalIntraCranialVol " ) ),
             data = imaging_iSiV )
    beta <- r$coefficients[ "EstimatedTotalIntraCranialVol" ]
    imaging_k_iSiV[ ,iV] <- imaging_iSiV[ ,iV] - beta * ( imaging_iSiV[ ,"EstimatedTotalIntraCranialVol"] - meanICV )
    #plot( imaging_iSiV[ ,iV],
    #      imaging_iSiV[ ,iV] - beta * ( imaging_iSiV[ ,"EstimatedTotalIntraCranialVol"] - meanICV ) )
  }
  
  # Keep only baseline data, eliminating duplicates
  imaging_k_iSiV[ ,"TimePoint"] <- gsub( "m",
                                       "",
                                       imaging_k_iSiV[ ,"TimePoint"] )
  imaging_k_iSiV[ ,"TimePoint"] <- as.numeric( imaging_k_iSiV[ ,"TimePoint"] )
  stopifnot( length( unique( imaging_k_iSiV[ ,"TimePoint"] ) ) == 4 )
  bGood_iS <- imaging_k_iSiV[ ,"TimePoint"] == 0
  imaging_ck_iSiV <- imaging_k_iSiV[ bGood_iS, ]
  imaging_ck_iSiV <- imaging_ck_iSiV[ duplicated( imaging_ck_iSiV[ ,"Individual"] ) == FALSE, ]
  dimnames( imaging_ck_iSiV )[[1]] <- imaging_ck_iSiV[ ,"Individual"]
  
  # Calculate brain decline
  bCalculate_iV <- grepl( "_volume$|MMSE_Score$|EstimatedTotalIntraCranialVol$",
                          dimnames( imaging_ck_iSiV )[[2]] )
  change_iSiV <- NA * imaging_ck_iSiV[ ,bCalculate_iV]
  dimnames( change_iSiV )[[1]] <- imaging_ck_iSiV[ ,"Individual"]
  stopifnot( all( duplicated( dimnames( change_iSiV )[[1]] ) == FALSE ) )
  stopifnot( all( dimnames( change_iSiV )[[1]] %in% imaging_k_iSiV[ ,"Individual"] ) )
  for( iS in 1:dim( change_iSiV )[[1]] ){
    
    # Select the subject
    bSubject_iS <- imaging_k_iSiV[ ,"Individual"] == dimnames( change_iSiV )[[1]][iS]
    if( mod( iS, 10 ) == 1 ){
      print( paste( "Calculating brain changes for subject", iS, "of", dim( change_iSiV )[[1]] ) )
    }
    
    # Calculate decline if there are enough datapoints
    for( iV in 1:dim( change_iSiV )[[2]] ){
      # Calculate chage and save
      kV <- dimnames( change_iSiV )[[2]][iV]
      if( sum( !is.na( imaging_k_iSiV[ bSubject_iS, kV ] ) ) > 1 ){
        r <- lm( formula = imaging_k_iSiV[ bSubject_iS, kV ] ~ imaging_k_iSiV[ bSubject_iS, "TimePoint" ] )    
        change_iSiV[iS,iV] <- r$coefficients[2]
      }else{
        print( paste( "Not enough non-NA datapoints for subject",
                      dimnames( change_iSiV )[[1]][iS] ))
        change_iSiV[iS,iV] <- NA
      }
    }
  }
  
  # Save data
  write.csv( file = "loadANM_imag_change.csv",
             x = change_iSiV )
  write.csv( file = "loadANM_imag_imaging.csv",
             x = imaging_k_iSiV )
  centre_iS <- sapply( imaging_k_iSiV[ ,"Individual"], function( s ){ substr( s, 1, 3 ) } )
  table(centre_iS)
  centre_iS <- sapply( dimnames( change_iSiV )[[1]], function( s ){ substr( s, 1, 3 ) } )
  table(centre_iS)
  
  # Merge data
  stopifnot( dim( imaging_ck_iSiV )[1] == dim( change_iSiV )[1] )
  stopifnot( all( dimnames( imaging_ck_iSiV )[[1]] == dimnames( change_iSiV )[[1]] ) )
  #dimnames( change_iSiV )[[2]] <- dimnames( imaging_ck_iSiV )[[2]][ bCalculate_iV ]
  dimnames( change_iSiV )[[2]] <- paste( "change_",
                                         dimnames( change_iSiV )[[2]],
                                         sep = "" )
  bGood_iV <- grepl( "_volume$",
                     dimnames( imaging_ck_iSiV )[[2]] )
  imaging_mck_iSiV <- cbind( imaging_ck_iSiV[ ,c( "Individual",
                                                  "MMSE_Score",
                                                  "APOEgenotype",
                                                  "EstimatedTotalIntraCranialVol" )],
                             imaging_ck_iSiV[ ,bGood_iV],
                             change_iSiV )
  
  # Align with the subjects that were requested
  dimnames( imaging_mck_iSiV )[[1]] <- fixSampleNames( nSs = dimnames( imaging_mck_iSiV )[[1]] );
  imaging_emck_iSiV <- array( data = NA,
                              dim = c( length( subjectID_iS ),
                                       dim( imaging_mck_iSiV )[2] ),
                              dimnames = list( subject = subjectID_iS,
                                               variable = dimnames( imaging_mck_iSiV )[[2]] ) )
  imaging_emck_iSiV <- as.data.frame( imaging_emck_iSiV )
  bExists_iS <- subjectID_iS %in% dimnames( imaging_mck_iSiV )[[1]]
  for( iS in which( bExists_iS ) ){
    bThis_iS <- dimnames( imaging_emck_iSiV )[[1]][iS] == dimnames( imaging_mck_iSiV )[[1]]
    stopifnot( sum( bThis_iS ) == 1 )
    imaging_emck_iSiV[iS, ] <- imaging_mck_iSiV[bThis_iS, ]
  }
  
  # Return results
  #save( file = "loadANM_imag.Rdata",
  #      list = c( "imaging_emck_iSiV" ) )
  return( imaging_emck_iSiV )
