...
# v8 = ks.test direction fixed
# v9 = buildBooleanMatrix added
#
...
buildBooleanMatrix = function( genes_jD ){
  #genes_jD = list( d1 = c("a","b","d"), d2 = c("b"), d3 = c("a","c"))
  
  # Check input
  if( length( names( genes_jD ) ) == 0 
      || is.null( names( genes_jD ) ) ){
    names( genes_jD ) <- 1:length( genes_jD );
  }
 
  # Create results structure
  nDs <- names( genes_jD );
  nGs <- unique( unlist( genes_jD ) );
  bGenes_iDiG <- array( data = FALSE,
                        dim = c( length( nDs ),
                                 length( nGs ) ),
                        dimnames = list( listPositions = nDs,
                                         listContents = nGs ) );
  
  # Set results structure
  stopifnot( length( nDs ) == length( genes_jD ) );
  for( iD in 1:length( nDs ) ){
    bGenes_iDiG[iD,genes_jD[[iD]]] <- TRUE;
  }
...
getHumanMouseHortologs = function( ){
  ...
  hort_iGi2 <- getLDS( attributes = c( "entrezgene" ), 
                       ...
                       martL = mouse );
  dimnames( hort_iGi2 )[[2]] <- c( "entrezHuman", "entrezMouse" )
  return( hort_iGi2 )
}
